...
def build_operation_center_snapshot(
    ...
    *,
    ...
    attention = tuple(
        record for record in all_records if record.status in ATTENTION_STATUSES
    ...
    return OperationCenterSnapshot(
        ...
        attention_operations=sum(record.status in ATTENTION_STATUSES for record in all_records),
